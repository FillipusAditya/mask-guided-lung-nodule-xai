...
def fill_post_dilation_holes(mask: np.ndarray) -> np.ndarray:
    """Fill every enclosed hole created or retained after dilation."""
    return ndi.binary_fill_holes(mask)
...
STAGE_FUNCTIONS = OrderedDict(
    [
        ...
        ("8. Fill holes after dilation", fill_post_dilation_holes),
    ]
)
...
def segment_image(
    ...
    if list(stage_config) != list(STAGE_FUNCTIONS):
        raise ValueError("stage_config must contain the eight stages in order.")
...
def process_batch(
    ...
    print(f"Lung segmentation | Total: {len(files)} | Written: {len(files)}")
