...
from config import LIDC_OUTPUT_DIR, LNDB_INPUT_DIR, LNDB_OUTPUT_DIR
...
def process_lidc_dataset(
    ...
    for index, scan in enumerate(scans, start=1):
        ...
        if output_path.exists() and not overwrite:
            print(f"[{index}/{len(scans)}] Skip {output_path.name}")
            continue

        print(f"[{index}/{len(scans)}] Process {scan.patient_id}")
...
def process_lndb_dataset(
    ...
    for index, mhd_path in enumerate(files, start=1):
        ...
        print(f"[{index}/{len(files)}] Process {mhd_path.name}")
...
def main() -> None:
    ...
    if args.dataset in ("lidc", "all"):
        ...
        print(f"LIDC-IDRI complete: {completed} processed, {total} available")
    ...
    if args.dataset in ("lndb", "all"):
        ...
        print(f"LNDb complete: {completed} processed, {total} available")
