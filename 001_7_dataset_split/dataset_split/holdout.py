...
from config import (
    COMBINED_ERROR_LOG_JSON as ERROR_LOG_JSON,
    COMBINED_HOLDOUT_CSV as SPLIT_METADATA_CSV,
    COMBINED_LABELED_METADATA_CSV as LABELED_METADATA_CSV,
    DATASET_DIR,
    DATA_DIRECTORIES,
    LIDC_LABEL_CSV,
    LNDB_LABEL_CSV,
...
def save_error_log(
    ...
    saved_samples: int = 0,
    output_json: str | Path = ERROR_LOG_JSON,
) -> None:
...
def print_summary(
    ...
    labeled_metadata_csv: str | Path = LABELED_METADATA_CSV,
    split_metadata_csv: str | Path = SPLIT_METADATA_CSV,
) -> None:
...
def run_combined_dataset_split() -> int:
    """Build labeled metadata and split both datasets by patient."""
    ...
    try:
        ...
        record_error(errors, "run_combined_dataset_split", error)
        print()
