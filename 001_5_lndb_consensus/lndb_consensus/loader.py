...
def prepare_scan_data(
    ...
    if len(rad_ids) != len(rad_finding_ids):
        raise ValueError(
            f"LNDb-{lndb_id:04d} finding {finding_id} has "
            f"{len(rad_ids)} radiologist IDs but "
            f"{len(rad_finding_ids)} radiologist finding IDs."
        )
