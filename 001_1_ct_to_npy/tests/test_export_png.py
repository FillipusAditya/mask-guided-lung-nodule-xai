"""Tests for deterministic CT-volume PNG export."""
...
from export_png import export_volume_file, window_to_uint8


class ExportPngTests(unittest.TestCase):
    def test_fixed_hu_window_maps_to_uint8(self) -> None:
        volume = np.array([[[-1500, -1400, -600, 200, 500]]], dtype=np.int16)

        result = window_to_uint8(volume, -1400, 200)

        np.testing.assert_array_equal(
            result,
            np.array([[[0, 0, 128, 255, 255]]], dtype=np.uint8),
        )
